...
def make_demo_path(T=700):
    ...
    x = 0.30 + 0.42 * u
    y = -0.22 + 0.46 * u + 0.08 * np.sin(2 * np.pi * u)
    z = 0.34 + 0.12 * np.sin(np.pi * u) + 0.04 * np.sin(2 * np.pi * u)
...
def main():
    ...
    anchor_idx1 = int(0.42 * T)
    anchor_idx2 = int(0.68 * T)

    spawn_idx1 = int(0.15 * T)
    spawn_idx2 = int(0.45 * T)
    ...
    obs1 = make_object_path(
        ...
        offset=(0.0, 0.18, 0.0),
        motion_fn=lambda t_local, i_local, anchor, offset: motion_cross_y(t_local, i_local, anchor, offset, v=-0.18),
        spawn_idx=spawn_idx1,
        keep_after_end=True
    )
    ...
    obs2 = make_object_path(
        ...
        offset=(0.09, -0.09, 0.07),
        motion_fn=lambda t_local, i_local, anchor, offset: motion_drift_diag(t_local, i_local, anchor, offset, v=(-0.05, 0.05, -0.06)),
        spawn_idx=spawn_idx2,
        keep_after_end=True
    )
...
#    anchor_idx=anchor_idx2,
#    offset=(0.0, -0.18, 0.0),
#    motion_fn=lambda t_local, i_local, anchor, offset:
#        motion_cross_y(t_local, i_local, anchor, offset, v=0.18),
#    spawn_idx=spawn_idx2,
#    keep_after_end=True
#)
